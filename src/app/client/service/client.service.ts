...
export class ClientService {
  ...
  constructor(private http: HttpClient) {
    this.clientUrl = 'http://localhost:8088/AGENT-SERVICE/agent';
    this.clientUrl1 = 'http://localhost:8088/AGENT-SERVICE/agent/client';

  }
  public findAll(code: string): Observable<Client[]> {
    return this.http.get<Client[]>(
      this.clientUrl+'/pointdevente/'  + code + '/clients'
    );
  }
